...
func TestGetPopularArticles_Exec(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockArticleRepository) {
				...
				m.EXPECT().GetPopularArticles(gomock.Any(), 5).Return(articles, nil)
			},
			wantLen:     1,
			wantErr:     false,
			expectLimit: 5,
		},
		{
			...
			setupMock: func(m *mocks.MockArticleRepository) {
				...
				m.EXPECT().GetPopularArticles(gomock.Any(), 10).Return(articles, nil)
			},
			wantLen:     10,
			wantErr:     false,
			expectLimit: 10,
		},
		{
			...
			setupMock: func(m *mocks.MockArticleRepository) {
				...
				m.EXPECT().GetPopularArticles(gomock.Any(), 20).Return(articles, nil)
			},
			wantLen:     20,
			wantErr:     false,
			expectLimit: 20,
		},
		{
			...
			setupMock: func(m *mocks.MockArticleRepository) {
				m.EXPECT().GetPopularArticles(gomock.Any(), 5).Return(nil, ErrRepository)
			},
			wantLen: 0,
			wantErr: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			uc := usecase.NewGetPopularArticles(mockRepo)

			got, err := uc.Exec(context.Background(), tt.input)

			if (err != nil) != tt.wantErr {
				t.Errorf("GetPopularArticles.Exec() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if len(got.Articles) != tt.wantLen {
				t.Errorf("GetPopularArticles.Exec() articles length = %v, want %v", len(got.Articles), tt.wantLen)
			}
		})
	}
}
